"use client";

import { FirebaseError } from "firebase/app";
import { signInWithPopup } from "firebase/auth";
import { useState, useTransition } from "react";
import { WatchSpinner } from "@/components/ui/watch-spinner";
import { googleLoginAction } from "@/lib/auth-actions";
import { auth, googleProvider } from "@/lib/firebase";

export function GoogleSignInButton() {
  const [pending, startTransition] = useTransition();
  const [error, setError] = useState<string | null>(null);

  async function handleClick() {
    setError(null);
    try {
      const credential = await signInWithPopup(auth, googleProvider);
      const idToken = await credential.user.getIdToken();
      startTransition(async () => {
        const result = await googleLoginAction(idToken);
        if (result?.error) setError(result.error);
      });
    } catch (err) {
      // Silently ignore the user closing / cancelling the popup.
      if (
        err instanceof FirebaseError &&
        (err.code === "auth/popup-closed-by-user" ||
          err.code === "auth/cancelled-popup-request")
      ) {
        return;
      }
      setError("No se pudo completar el inicio con Google.");
    }
  }

  return (
    <div className="flex flex-col gap-2">
      <button
        type="button"
        onClick={handleClick}
        disabled={pending}
        className="flex h-11 items-center justify-center gap-3 rounded-lg border border-border bg-surface-2 font-medium hover:border-accent disabled:opacity-50 transition-colors"
      >
        {pending ? <WatchSpinner className="w-[18px] h-[18px]" /> : <GoogleIcon />}
        {pending ? "Conectando..." : "Continuar con Google"}
      </button>
      {error && <p className="text-sm text-danger">{error}</p>}
    </div>
  );
}

function GoogleIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 18 18" aria-hidden="true">
      <path
        fill="#4285F4"
        d="M17.64 9.2c0-.64-.06-1.25-.16-1.84H9v3.48h4.84a4.14 4.14 0 0 1-1.8 2.72v2.26h2.92c1.7-1.57 2.68-3.88 2.68-6.62Z"
      />
      <path
        fill="#34A853"
        d="M9 18c2.43 0 4.47-.8 5.96-2.18l-2.92-2.26c-.8.54-1.84.86-3.04.86-2.34 0-4.32-1.58-5.03-3.7H.96v2.33A9 9 0 0 0 9 18Z"
      />
      <path
        fill="#FBBC05"
        d="M3.97 10.72a5.4 5.4 0 0 1 0-3.44V4.95H.96a9 9 0 0 0 0 8.1l3.01-2.33Z"
      />
      <path
        fill="#EA4335"
        d="M9 3.58c1.32 0 2.5.46 3.44 1.35l2.58-2.58C13.47.9 11.43 0 9 0A9 9 0 0 0 .96 4.95l3.01 2.33C4.68 5.16 6.66 3.58 9 3.58Z"
      />
    </svg>
  );
}
